...
bool SerialComm::Open(const string& portname, int baud_rate, int dataBit, int parity, int stopBit, int flowCtl) {
	if (port_.is_open()) return true;
	error_code ec;
	port_.open(portname, ec);
	if (!ec) {
		port_.set_option(serial_port::baud_rate(baud_rate));
		port_.set_option(dataBit == 7 ? serial_port::character_size(7) : serial_port::character_size(8));
		port_.set_option(parity == 1 ? serial_port::parity(serial_port::parity::odd) :
			(parity == 2 ? serial_port::parity(serial_port::parity::even) :
				serial_port::parity(serial_port::parity::none)));
		port_.set_option(stopBit == 1 ? serial_port::stop_bits(serial_port::stop_bits::onepointfive) :
			(stopBit == 2 ? serial_port::stop_bits(serial_port::stop_bits::two) :
				serial_port::stop_bits(serial_port::stop_bits::one)));
		port_.set_option(flowCtl == 1 ? serial_port::flow_control(serial_port::flow_control::software) :
			(flowCtl == 2 ? serial_port::flow_control(serial_port::flow_control::hardware) :
...
int SerialComm::Lookup(char* first) {
	mutex_lock lck(mtxrcv_);
	int n(crcrcv_.size());
	if (first && n) *first = crcrcv_[0];
	return n;
}
